...
import models.LineActivitiesModel;
...
public class OrderController extends Controller implements IConst {
	...
	public static Result addOrder() {
		...
		if (!httpForm.hasErrors()) {
			...
			if (Math.abs(amount - formObj.productAmount) > 1) {// 与json商品总额比对
				play.Logger.error(String.format("*******新增订单商品总额复核异常, 须检查. 原商品额: %s, 计算结果: %s, 发起IP: %s",
						formObj.productAmount, amount, request().remoteAddress()));
	...
	public static Result addOrderZero(long LineActivitiesId) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		Msg<OrderModel> msg = new Msg<>();

		LineActivitiesModel found = LineActivitiesModel.find.byId(LineActivitiesId);
		Form<OrderModel> httpForm = form(OrderModel.class).bindFromRequest();
	...
	public static Result deleteOrder(long id) {
		...
		if (found != null) {
			...
			for (ProductModel order : found.orderProducts) {
				order.orders.remove(found);
				Ebean.update(found);
			}
			found.orderProducts = new ArrayList<>();
			Ebean.update(found);
			Ebean.delete(found);
	...
	public static Result getAllOrders(Integer status, String keyword, Integer page, Integer size) {
		...
		if (status == -1) {
			if (StrUtil.isNull(keyword)) {
				records = OrderModel.find.orderBy("id desc").findPagingList(size).setFetchAhead(false)
						.getPage(page - 1);
			} else {
			...
			if (!StrUtil.isNull(order.payTime)) {
				...
				if (MillSeconds >= payMillSeconds) {
					...
					if (order.buyer != null) {
						if (!StrUtil.isNull(order.buyer.wxOpenId)) {
							try {
								String wxMessageText = "亲，由于您在七天之内没有确认，系统已经自动帮您确认！订单号为：" + orderno.toUpperCase();
	...
	@Security.Authenticated(Secured.class)
	public static Result getOrdersByUser(Integer status, long refBuyerId, Integer page, Integer size) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		if (size == 0)
			size = PAGE_SIZE;
		...
		if (found != null) {
			...
			if (status == -1) {
				records = OrderModel.find.where().eq("refBuyerId", refBuyerId).orderBy("id desc").findPagingList(size)
						.setFetchAhead(false).getPage(page - 1);
			} else {
				records = OrderModel.find.where().and(Expr.eq("refBuyerId", refBuyerId), Expr.eq("status", status))
						.orderBy("id desc").findPagingList(size).setFetchAhead(false).getPage(page - 1);
			}
	...
	@Security.Authenticated(Secured.class)
	public static Result getOrdersByUserNum(Integer status, long refBuyerId,long storeId, Integer page, Integer size) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		if (size == 0)
			size = PAGE_SIZE;
	...
	public static Result getResellerOrderAmount(Long id) {
		...
		play.Logger.info("valid order count: " + orders.size());
