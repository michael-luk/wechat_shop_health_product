...
public class UserController extends Controller implements IConst {
	...
	public static Result setReseller(long id) {
		play.Logger.info(DateUtil.Date2Str(new Date()) + " - " + request().method() + ": " + request().uri()
				+ " | DATA: " + request().body().asJson());
		Msg<UserModel> msg = new Msg<>();
	...
	public static Result handleWeChatBarcodeUsers() {
		Msg msg = new Msg();

		List<UserModel> list = UserModel.find.where().isNull("resellerCodeImage").isNotNull("resellerCode").findList();

		if (list.size() <= 0) {
			msg.message = "no users with issue";
			return ok(Json.toJson(msg));
		}

		for (UserModel user : list) {
			try {
				String barcodeFilename = WeiXinController.generateResellerCodeBarcode(user.resellerCode);
				if (StrUtil.isNotNull(barcodeFilename)) {
					user.resellerCodeImage = barcodeFilename;
					Ebean.update(user);
					play.Logger.info("renew user barcode success: " + user.id.toString() + ", " + user.resellerCode);
				} else {
					play.Logger.info("renew user barcode fail: download failed.");
				}
			} catch (Exception ex) {
				play.Logger.error("renew user barcode fail: " + user.id.toString() + ", ex:" + ex.getMessage());
			}
		}
		msg.flag = true;
		return ok(Json.toJson(msg));
