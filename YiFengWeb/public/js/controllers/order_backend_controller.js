...
app.controller('OrderBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    function refreshDate() {
        var url = '/orders?size=5&page=' + $scope.page + '&keyword=' + $scope.findOrder
    ...
    $scope.gridOptions = {
        ...
        columnDefs: [
           ...
           // {field: 'themeName', displayName: '产品口味', width: '70'},
            ...
            {field: 'promotionAmount', displayName: '尊享码优惠', width: '80'},
            {
                field: 'comment',
                displayName: '活动优惠',
                width: '80',
           ...
           //{field: 'shipCity', displayName: '城市', width: '80'},
            //{field: 'shipZone', displayName: '区域', width: '200'},
            {field: 'shipLocation', displayName: '详细地址', width: '250', cellTemplate: '<div ng-bind-html="COL_FIELD | safehtml"></div>'},
            ...
            {field: 'reseller',displayName: '上线会员号',width: '140',cellTemplate: '<div ng-bind-html="COL_FIELD.resellerCode | safehtml"></div>'},
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        $http.delete('/orders/' + content.id).success(function(data, status, headers, config) {
                            if (data.flag) {
