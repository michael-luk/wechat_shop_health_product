        ...
        $http.get('/users/current/login').success(
            function(data, status, headers, config) {
                if (data.flag) {
                    ...
                    $http.get('/ordersnum/user/' + $scope.user.id + '?size=10000000').success(
                        function (data, status, headers, config) {
                            if (data.flag) {
                                $scope.myOrdersNum = data.code
...
//  			  alert(data.message);
      ...
      /*  function calculatePromotion(promList){
        ...
        function calculatePromotion(promList) {
            ...
            for (x in promList) {
                if (promList[x].available) {//TODO: 以后要计算复杂促销
                    if(promList[x].promotionType == "普通优惠"){
                        discount -= promList[x].discount
                    }
                    if(promList[x].promotionType == "首单优惠" &&  $scope.myOrdersNum < 1){
                        discount -= promList[x].discount
                    }

                    /*  if(promList[x].promotionType == "免运费" && $scope.newOrder.productAmount >= promList[x].reachMoney){
                     discount -= promList[x].discount
                     }*/
                }
            }
            //选择满足条件中最优惠的一种
            for (j in promList) {
                if (promList[j].available) {
                    if (promList[j].promotionType == "满减优惠" && $scope.newOrder.productAmount >= promList[j].reachMoney) {
                        if (promList[j].reachMoney >= FullReduction) {
                            FullReduction = promList[j].reachMoney
                            PreferentialAmount = promList[j].discount
                            $scope.promotionId = promList[j].id;
                        }
                    }
                }
            }
            discount -= PreferentialAmount
            $scope.newOrder.comment = 0
            $scope.newOrder.comment -= discount
            return discount
        }

        //判断是否免运费
        function FreeShippingPromotion(promList ) {
            var discount = 0
            for (x in promList) {
                if (promList[x].available) {
                    if(promList[x].promotionType == "免运费" && $scope.newOrder.productAmount >= promList[x].reachMoney){
                    ...
                    function (data, status, headers,
                        ...
                        if (data.flag) {
                          ...
                          /*  if($scope.newOrder.productAmount < $scope.shipInfo.area * 1 && $scope.Since == false ){
                            ...
                            if(FreeShippingPromotion($scope.promotions)){
