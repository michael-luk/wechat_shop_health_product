...
var uploadDescription = '<div> <input type="file" name="files[]" accept="image/*" ng-file-select="uploadImage($files, \'descriptionImages\', row.entity)"/> <div ng-repeat="descriptionName in row.entity.descriptionImagesList"> <a class="fancybox" target="_blank" data-fancybox-group="gallery" fancybox ng-if="isShowImg(descriptionName)" ng-href="/showImage/{{descriptionName}}" ><img ng-src="/showimg/thumb/{{descriptionName}}" style="width:50px;height:50px;float:left"></a><input type="button" ng-if="isShowImg(descriptionName)" ng-click="deleteDescription(row.entity, \'descriptionImages\', descriptionName)" value="删除" style="float:left" /></div></div>';
...
app.controller('ProductBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    function refreshDate(){
        var url = '/products?page=' + $scope.page + '&keyword=' + $scope.findProduct + '&catalogId=' + $scope.selectCatalogId
        $log.log(url)
        ...
        $http.get(url).success(function (data, status, headers, config) {
            ...
            if (data.flag) {
                for(x in data.data){
                    ...
                    if(data.data[x].descriptionImages){
                        data.data[x].descriptionImagesList = data.data[x].descriptionImages.split(',')
                    }
                    else{
                        data.data[x].descriptionImagesList = ''
                    }
                }
    ...
    $scope.gridOptions = { data: 'list',
        ...
        columnDefs: [
            ...
            {field: 'inventory', displayName: '库存', width: '60', enableCellEdit: true, editableCellTemplate: cellEditableTemplateProduct},
            {field: 'imageList', displayName: '产品图', width: '190', cellTemplate: uploadTemplateProduct},
            {field: 'descriptionImagesList', displayName: '产品详情图', width: '460', cellTemplate: uploadDescription},
            {field: 'shortDesc', displayName: '产品简述', width: '100', enableCellEdit: true, editableCellTemplate: cellEditableTemplateProduct},
            {field: 'price', displayName: '价格', width: '70', enableCellEdit: true, editableCellTemplate: cellEditableTemplateProduct},
            {field: 'ml', displayName: '净含量', width: '70', enableCellEdit: true, editableCellTemplate: cellEditableTemplateProduct},
            {field: 'isActiveProduct', displayName: '是否设为活动产品', width: '120', cellTemplate: '<div><input type="checkbox" ng-model="COL_FIELD" ng-click="updateHotSale(row.entity)" /></div>'},
            {field: 'isHotSale', displayName: '是否设为热销', width: '120', cellTemplate: '<div><input type="checkbox" ng-model="COL_FIELD" ng-click="updateHotSale2(row.entity)" /></div>'},
            ...
            {field: 'comment', displayName: '备注', width: '100'},
        ] };
            ...
            $scope.upload = $upload.upload({
                ...
                .success(function(data, status, headers, config) {
                    ...
                    if (data.flag) {
                        if (imageField == 'images') {
                        ...
                        else if (imageField == 'descriptionImages') {
                            parentObj.description = parentObj[imageField].split(',');
                            if(parentObj[imageField])
                                     for(i in parentObj.description){
                                                if(parentObj.description[i] == data.data)
                                                {
                                                    parentObj.description =  null;
                                                    break
                                            }
                                        }
                            if(parentObj.description != null){
                                parentObj[imageField] += ',' + data.data;
                            }
                           ...
                           /* parentObj[imageField] += ',' + data.data;*/
                                ...
                                parentObj[imageField] = data.data;
                            parentObj.descriptionImagesList = parentObj[imageField].split(',');
                        }
                        else {
    ...
    $scope.deleteDescription = function(obj, property, descriptionName) {
        $scope.currentObj = obj;
        var index = obj.descriptionImagesList.indexOf(descriptionName)
        obj.descriptionImagesList.splice(index, 1)// 在数组中删掉这个图片文件名
        obj[property] = obj.descriptionImagesList.join(",")// 数组转为字符串, 以逗号分隔
    ...
    // 产品是否热销
    $scope.updateHotSale2 = function(obj, checkStatus) {
